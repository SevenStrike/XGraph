    ...
    public class visualnode_s_in_1port_m_out_1port : visualnode_base
    {
        ...
        private void On_CreateEdge(GraphViewChange graphViewChange)
        {
            if (graphViewChange.edgesToCreate != null)
            {
                ...
                {
                    // 如果该视觉节点的行为数据类型为： composite
                    if (ActionNode is actionnode_composite composite)
                    {
                        visualnode_base n_parent = edge.output.node as visualnode_base;
                        // edge 连线的子级节点
                        visualnode_base n_child = edge.input.node as visualnode_base;

                        if (composite.guid == n_parent.ActionNode.guid && n_child != null)
                        {
                            bool existChild = false;
                            composite.childNodes.ForEach(c =>
                            {
                                if (n_child.ActionNode.guid == c.guid)
                                    existChild = true;
                            });
                            if (existChild)
                            {
                            ...
                            Undo.RecordObject(composite, "Create_Composite_ChildNode");
                            composite.childNodes.Add(n_child.ActionNode);
                        }
                    }
                });
            }
        ...
        private void Remove_Edge(GraphElement element)
        {
            ...
            if (edge != null)
            {
                ...
                if (ActionNode is actionnode_composite composite)
                {
                    ...
                    {
                        // 说明删除的连线的输出端口的节点是 Composite 节点
                        if (ActionNode.guid == n_parent.ActionNode.guid)
                        {
                            Undo.RecordObject(composite, "Remove_Composite_ChildNode");
                            composite.childNodes.Remove(n_child.ActionNode);
                        }
                    }
                }
                #endregion
        ...
        private void Remove_Node(GraphElement element)
        {
            ...
            if (remove_node != null)
            {
                ...
                {
                    Undo.RecordObject(graphView.ActionTreeAsset, "Remove_Composite_Node");
                ...
                if (ActionNode is actionnode_composite composite)
                {
                    // 如果删除的节点包含在合成节点的子节点列表中
                    if (composite.childNodes.Contains(remove_node.ActionNode))
                    {
                        Undo.RecordObject(composite, "Remove_Composite_ChildNode");
                        // 从合成节点的子节点列表中移除删除的节点
                        composite.childNodes.Remove(remove_node.ActionNode);
                    }
                }
            }
        }
