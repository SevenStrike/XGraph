        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            if (evt.target is xg_GraphView graphview)
            {
                ...
                if (ActionTreeAsset.GraphViewCustomBG_Texture != null)
                {
                    ...
                    string[] opacityLevel_Names = new string[5] { "E 浅影", "R 浮现", "T 半透明", "Y 清晰", "U 不透明" };
                    float[] opacityLevel_Value = new float[5] { 0.1f, 0.25f, 0.4f, 0.6f, 1f };

                    for (int i = 0; i < opacityLevel_Names.Length; i++)
                    {
