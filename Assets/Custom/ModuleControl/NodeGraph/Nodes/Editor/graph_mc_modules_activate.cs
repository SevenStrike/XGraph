namespace SevenStrikeModules.XGraph
{
    ...
    public class graph_mc_modules_activate : xNode_Base
    {
        action_mc_modules_activate activate;
        ...
        public override void Initialize(xg_GraphView graphView, Vector2 pos = default, xAction_Base data = null)
        {
            ...
            activate = data as action_mc_modules_activate;
        }
        ...
        public override void On_VariablesValue_Changed()
        {
            ...
            activate.Set_ActivateModulesState("激活");
        }
        /// <summary>
        /// 当克隆节点时
        /// </summary>
        ...
        public override Foldout ins_Folder_ChildActions(VisualElement root)
        {
            ...
            fold.text = $"{fold.text}（{activate.childNodes.Count}）";

            for (int i = 0; i < activate.childNodes.Count; i++)
            {
                xAction_Base child = activate.BaseArgs.RootAsset.FindActionNode(activate.childNodes[i]);
        ...
        public override Foldout ins_Folder_Extensions(VisualElement root)
        {
            ...
            #region  启动时所有模组的激活开关
            Toggle toggle = util_XGraphInspectorGUI.GUI_Field_Bool(fold, "激活", activate.activateState, new string[] { "field_bool" });
            toggle.RegisterValueChangedCallback((value) =>
            {
                // 如果该节点已经存在绑定的变量，则将当前序列化值给到控件值，因为该序列化值已受变量控制
                if (isVariableBinded("激活"))
                {
                    toggle.value = activate.activateState;
                    activate.Set_ActivateModulesState("激活");
                }
                // 否则就代表没有任何变量节点接入，而使用控件值给到序列化属性值
                else
                {
                    activate.Set_ActivateModulesState(value.newValue);
                }
            });
            #endregion

            // 当节点绑定变量时，将变量值同步到控件值
            activate.On_Node_Variable_Binded += ((vare) =>
            {
                toggle.value = activate.activateState;
            });

            // 克隆节点后刷新控件值为克隆后的最新值
            activate.On_Node_Duplicated += (clone, source) =>
            {
                // 克隆父物体的行为数据
