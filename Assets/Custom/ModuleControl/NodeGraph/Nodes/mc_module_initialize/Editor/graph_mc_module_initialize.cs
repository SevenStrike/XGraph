namespace SevenStrikeModules.XGraph
{
    ...
    public class graph_mc_module_initialize : xNode_Base
    {
        ...
        public override Foldout ins_Folder_ChildActions(VisualElement root)
        {
            ...
            fold.text = $"{fold.text}（{module_Initialize.childNodes.Count}）";

            for (int i = 0; i < module_Initialize.childNodes.Count; i++)
            {
                xAction_Base child = module_Initialize.RootAsset.FindActionNode(module_Initialize.childNodes[i]);
        ...
        public override Foldout ins_Folder_Extensions(VisualElement root)
        {
            ...
            toggle.RegisterValueChangedCallback((value) =>
            {
                ...
                {
                    module_Initialize.activateAllModules = value.newValue;
                }
                module_Initialize.Set_ModulesInitialized("激活所有模组");
            });
            #endregion
